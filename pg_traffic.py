...
class pg_traffic_analyzer:
    def __init__(self, iface_name, pg_server_addr, pg_server_port, train_mode=None):
        """
        Analyser for Postgre network traffic.
        Requires scapy and netifaces

        :param iface_name: Network interface name
        :param pg_server_addr: Postgre server's IP-address(string parameter)
        :param pg_server_port: Postgre server's port
        ...
        self.__pg_server_addr = pg_server_addr
        self.__pg_server_port = str(pg_server_port)
        self.__train_mode = train_mode[0]
        self.__even_logger = db_event_ui('Postgres')
    ...
    def __find_audit_row(payload, data_vectors):
        """
        Matches exact Postgre audit record and network packet

        :param payload: Postgre-protocol packet payload
        :param data_vectors: Set of candidates from Postgre audit table
        :return: index of Postgre audit record
        :rtype: str
        """
    ...
    def __create_packet_info_struct(self, packet):
        ...
        :param packet: Postgre packet of query/response
        ...
        return [self.__query_index,
                ...
                'TCP ' + str(packet[scapy.IP].src) + ':' + str(
                    packet[scapy.TCP].sport) + " > " + self.__pg_server_addr + ":" + self.__pg_server_port,
                payload,
                ]

    def __analyze_packet(self, packet):
        """
        Picks query and response Postgre packets and forms data vectors for future ML analysis

        :param packet: Network Postgre packet
        """
        payload = bytes(packet.payload.payload.payload)
    ...
    def __process_packets(self, packet):
        """
        Saves Postgre packets and updates session list
    ...
    def sniff_packets(self):
        """
        Sniffs Postgre packets in local network
        """
        filter_str = '(dst port ' + self.__pg_server_port + ' or src port ' + self.__pg_server_port + ') and tcp and ' + '(not (host ' + self.__ids_ip + '))'
