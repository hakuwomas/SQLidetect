...
class mysql_traffic_analyzer:
    def __init__(self, iface_name, mysql_server_addr, mysql_server_port, train_mode=None):
        """
        Analyser for MySQL network traffic.
        Requires scapy and netifaces

        :param iface_name: Network interface name
        :param mysql_server_addr: MySQL server's IP-address(string parameter)
        :param mysql_server_port: MySQL server's port
        ...
        self.__mysql_server_addr = mysql_server_addr
        self.__mysql_server_port = str(mysql_server_port)
        self.__train_mode = train_mode[0]
        self.__even_logger = db_event_ui('MYSQL')
    ...
    def __find_audit_row(payload, data_vectors):
        """
        Matches exact MySQL audit record and network packet

        :param payload: MySQL-protocol packet payload
        :param data_vectors: Set of candidates from MySQL audit table
        :return: index of MySQL audit record
        :rtype: str
        """
    ...
    def __create_packet_info_struct(self, packet):
        ...
        :param packet: MySQL packet of query/response
        ...
        return [self.__query_index,
                ...
                'TCP ' + str(packet[scapy.IP].src) + ':' + str(
                    packet[scapy.TCP].sport) + " > " + self.__mysql_server_addr + ":" + self.__mysql_server_port,
                payload,
                ]

    def __analyze_packet(self, packet):
        """
        Picks query and response MySQL packets and forms data vectors for future ML analysis

        :param packet: Network MySQL packet
        """
        payload = bytes(packet.payload.payload.payload)
    ...
    def __process_packets(self, packet):
        """
        Saves MySQL packets and updates session list

        :param packet: Captured MySQL packet
        """        
    ...
    def sniff_packets(self):
        """
        Sniffs MySQL packets in local network
        """
        filter_str = '(dst port ' + self.__mysql_server_port + ' or src port ' + self.__mysql_server_port +') and tcp and ' + '(not (host ' + self.__ids_ip + '))'
