...
class mssql_traffic_analyzer:
    def __init__(self, iface_name, mssql_server_addr, mssql_server_port, train_mode=None):
        """
        Analyser for mssql network traffic.
        Requires scapy and netifaces

        :param iface_name: Network interface name
        :param mssql_server_addr: mssql server's IP-address(string parameter)
        :param mssql_server_port: mssql server's port
        ...
        self.__mssql_server_addr = mssql_server_addr
        self.__mssql_server_port = str(mssql_server_port)
        self.__train_mode = train_mode[0]
        self.__even_logger = db_event_ui('mssql')
    ...
    def __find_audit_row(payload, data_vectors):
        """
        Matches exact mssql audit record and network packet

        :param payload: TDS-protocol packet payload
        :param data_vectors: Set of candidates from mssql audit table
        :return: index of mssql audit record
        :rtype: str
        """
    ...
    def __create_packet_info_struct(self, packet):
        ...
        :param packet: TDS packet of query/response
        ...
        return [self.__query_index,
                ...
                'TCP ' + str(packet[scapy.IP].src) + ':' + str(
                    packet[scapy.TCP].sport) + " > " + self.__mssql_server_addr + ":" + self.__mssql_server_port,
                payload,
                ]

    def __analyze_packet(self, packet):
        """
        Picks query and response TDS packets and forms data vectors for future ML analysis

        :param packet: Network TDS packet
        """
        payload = bytes(packet.payload.payload.payload)
    ...
    def __process_packets(self, packet):
        """
        Saves TDS packets and updates session list

        :param packet: Captured TDS packet
        """        
    ...
    def sniff_packets(self):
        """
        Sniffs TDS packets in local network
        """
        filter_str = '(dst port ' + self.__mssql_server_port + ' or src port ' + self.__mssql_server_port + ') and tcp and ' + '(not (host ' + self.__ids_ip + '))'
